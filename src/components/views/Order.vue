<template>
  <div>
    ...
    <div class="container">
      ...
      <el-table :data="orderData" border class="table" ref="multipleTable" header-cell-class-name="table-header" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="Indate" label="入住时间"></el-table-column>
        <el-table-column prop="Days" label="入住天数"></el-table-column>
        <el-table-column prop="Remark" label="备注" :formatter="if_null"></el-table-column>
        <el-table-column label="操作" width="180" align="center">
          <template slot-scope="scope">
            <el-button type="text" icon="el-icon-edit" @click="handleEdit(scope.$index, scope.row)">入住</el-button>
            <el-button type="text" icon="el-icon-delete" class="red" @click="handleDelete(scope.$index, scope.row)">取消</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <!-- 编辑弹出框 -->
    <el-dialog title="确定信息" :visible.sync="editVisible" width="30%">
      <el-form ref="form" :model="form" label-width="70px">
        <el-form-item label="入住人名">
...
export default {
    ...
    methods: {
        ...
        getData() {
            var _this = this;
            this.$http.post('/selectBookInOrder',{UserName:_this.query.name}).then(function(res) {
                console.log(res.data);
                res.data.forEach(element => {
                  //处理两个时间、订单时间和入住时间
                  element.Indate = element.Indate.split('T')[0]
                  element.orderTime = element.orderTime.split('T')[0]
                });
